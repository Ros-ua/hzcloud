        ...
        async def handle_specific_user_messages(event):
            if event.is_private:  # Проверяем, что сообщение пришло из личного чата
                ...
                if "_банка" in message_text or "_банку" in message_text or "_пить" in message_text:
                    ...
                    await self.client.send_message(self.bot_id, "а вот эта строка это тест. приём приём ")
                    ...
                    if self.last_energy_message:  # Проверяем, что last_energy_message не None
                        if self.is_in_caves:
                            forwarded_msg = await self.last_energy_message.forward_to(self.group59)  # Пересылаем и сохраняем ссылку
                        else:    
                            forwarded_msg = await self.last_energy_message.forward_to(1033007754)  # Пересылаем и сохраняем ссылку
                        ...
                        sent_msg = await self.client.send_message(self.group59, "ещё не капнуло")  # Отправляем и сохраняем ссылку
