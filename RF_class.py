...
class RF:
            ...
            if event.is_private:  # Проверяем, что сообщение пришло из личного чата
                ...
                # Проверяем, что отправитель не является cave leader
                  ...
                  # Находим соответствующую команду
                    ...
                    await event.message.delete()  # Удаляем сообщение
                    msg = await self.client.send_message(event.chat_id, "Ver.4.7.11")
                    await asyncio.sleep(5)
    ...
    async def handle_antiki_command(self, event):
        ...
        for idx, recipe in enumerate(recipes, 1):
            ...
            # Ждём НОВОЕ сообщение от бота (максимум 10 секунд)
            detail_msg = None
            ...
            if not detail_msg:
                print(f"  ⚠ Пропуск: не получен ответ за 10 секунд")
                continue
